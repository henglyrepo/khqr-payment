...
class AsyncKHQRClient:
    """Async client for Bakong KHQR API."""
    ...
    async def __aenter__(self) -> "AsyncKHQRClient":
        return self
    ...
    async def check_bulk_payments(self, md5_list: list[str]) -> BulkPaymentStatus:
        ...
        try:
            response = await self._client.post(endpoint, json={"hashes": md5_list})
    ...
    async def is_connected(self) -> bool:
        """Check if client is connected to API (async)."""
        try:
