...
class TestQRParser:
    def test_parse_valid_qr_string(self):
        ...
        assert parsed["merchant_id"] == "user@bank"
...
class TestValidators:
    ...
    def test_validate_invalid_currency(self):
        with pytest.raises(CurrencyError):
            validate_request(amount=100.00, currency="EUR", bank_account="user@bank", is_static=False)
    ...
    def test_validate_exceed_max_amount_usd(self):
        with pytest.raises(AmountError):
            validate_request(amount=1000000, currency="USD", bank_account="user@bank", is_static=False)
